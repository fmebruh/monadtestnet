...
def wrap_mon(private_key, amount):
    try:
        ...
        print_step('wrap', f"Estimated Gas: {tx['gas']}, Gas Price: {w3.from_wei(tx['gasPrice'], 'gwei')} Gwei")
        # --- End of Fix ---
...
def unwrap_mon(private_key, amount):
    try:
        ...
        gas_estimate = w3.eth.estimate_gas(tx)
        tx['gas'] = int(gas_estimate * 1.2)
        print_step('unwrap', f"Estimated Gas: {tx['gas']}, Gas Price: {w3.from_wei(tx['gasPrice'], 'gwei')} Gwei")
        # --- End of Fix ---
...
def swap_mon_to_usdt(private_key, amount):
    try:
        ...
        # --- FIX: Dynamic Gas Estimation for Approval ---
        ...
        gas_estimate_approve = w3.eth.estimate_gas(approve_tx)
        approve_tx['gas'] = int(gas_estimate_approve * 1.2)
        print_step('swap', f"Approval Gas: {approve_tx['gas']}, Gas Price: {w3.from_wei(approve_tx['gasPrice'], 'gwei')} Gwei")
        # --- End of Fix ---
        ...
        gas_estimate = w3.eth.estimate_gas(tx)
        tx['gas'] = int(gas_estimate * 1.2) # Increased buffer to 20% for complex swaps
