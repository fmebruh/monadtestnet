...
async def stake_mon(account, private_key, cycle_number):
    try:
        ...
        print_step('stake', f"Stake Amount: {Fore.GREEN}{w3.from_wei(stake_amount, 'ether')} MON{Style.RESET_ALL}")
        ...
        print_step('stake', f"Estimated Gas: {tx['gas']}")
        # --- End of Fix ---
...
async def request_unstake_apr_mon(account, private_key, amount_to_unstake, cycle_number):
    try:
        ...
        gas_estimate = w3.eth.estimate_gas(tx)
        tx['gas'] = int(gas_estimate * 1.2)
        print_step('unstake', f"Estimated Gas: {tx['gas']}")
        # --- End of Fix ---
...
async def claim_mon(account, private_key, cycle_number):
    try:
        ...
        gas_estimate = w3.eth.estimate_gas(tx)
        tx['gas'] = int(gas_estimate * 1.2)
        print_step('claim', f"Estimated Gas: {tx['gas']}")
        # --- End of Fix ---
