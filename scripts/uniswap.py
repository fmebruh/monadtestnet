...
async def approve_token(private_key, token_address, amount, token_symbol):
    ...
    async def do_approve():
        ...
        print_step('approve', f"Estimated Gas: {tx['gas']}")
        # --- End of Fix ---
...
async def swap_eth_for_tokens(private_key, token_address, amount_in_wei, token_symbol):
    ...
    async def do_swap():
        print_border(f"Swapping {w3.from_wei(amount_in_wei, 'ether')} MON to {token_symbol} | {account.address[:8]}...", Fore.MAGENTA)
        ...
        gas_estimate = w3.eth.estimate_gas(tx)
        tx['gas'] = int(gas_estimate * 1.2)
        print_step('swap', f"Estimated Gas: {tx['gas']}")
        # --- End of Fix ---
...
async def swap_tokens_for_eth(private_key, token_address, token_symbol):
    ...
    async def do_swap():
        print_border(f"Swapping {token_symbol} to MON | {account.address[:8]}...", Fore.MAGENTA)
